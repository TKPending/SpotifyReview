...
const RemoveAccessButton = () => {
  ...
  return (
    ...
    >
      <p onClick={confirmRemoveAccess} className="text-white text-base font-semibold p-4 rounded-lg bg-green-600 hover:bg-green-800 transition duration-200 hover:cursor-pointer">
        Remove Access
      </p>

      {popup && (
        <div className="absolute top-16 text-white flex gap-6 transition duration-600">
          <p onClick={() => handleRemoveAccess(true)} className="hover:text-green-300 text-xs hover:cursor-pointer hover:underline transition duration-200">Confirm</p>
          <p onClick={() => handleRemoveAccess(false)} className="hover:text-red-200 text-xs hover:cursor-pointer hover:underline transition duration-200">Decline</p>
        </div>
      )}
    </div>
  );
